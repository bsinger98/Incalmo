...
class LogicalPlanner:
    def __init__(
        ...
        # Assumes attackers knows about both subnets
    ...
    async def initialize(self):
        ...
        self.knowledge_base.add_infected_host(self.attacker_agent)
    ...
    async def initial_access(self):
        ...
        if attacker_host is None:
            log_event("Initialization", "Error, attacker host not found")
            self.stopping_condition_met = True
            self.next_bucket = None
            return
    ...
    async def random_spread(self):
        ...
        if len(self.attack_path_queue) > 0:
            random_index = random.randint(0, len(self.attack_path_queue) - 1)
            attack_path = self.attack_path_queue.pop(random_index)
